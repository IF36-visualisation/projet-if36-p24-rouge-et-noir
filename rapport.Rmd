---
title: "Rapport --- découvrez la performance du chiffre d'affaires d’un supermarché"
...
- **[Source 🚀](https://www.kaggle.com/aksha17/superstore-sales)**

```{r echo=FALSE, results='asis'}
data_description <- data.frame(
  Caractéristique = c("invoice_id", "branch", "city", "customer_type", "gender", "product_line", "unit_price", "quantity", "vat", "total", "dtme", "tme", "payment_method", "cogs", "gross_margin_pct", "gross_income", "rating", "time_of_day", "day_name", "month_name"),
  Description = c(
    "Identifiant de la facture", 
    "Le symbol de ce chaîne de Walmart", 
    "La ville où se trouve le Walmart", 
    "Membre ou non (carte de fidélité)", 
    "Sexe de ce client", 
    "Catégorie de cet article", 
    "Prix d’un pièce", 
    "Quantité des articles achetés", 
    "TVA en France", 
    "Montant total d’achat", 
    "Date et l'heure de l'achat", 
    "Heure précise de l’achat", 
    "Mode de paiement, comme espèces", 
    "Coût d'un produit vendu", 
    "Marge brute en pourcentage(%)", 
    "Revenu brut total", 
    "Évaluation de l'expérience d'achat", 
    "Moment d’achat, comme matin/midi", 
    "Jour de la semaine", 
...
# Utilisez la fonction kable pour créer un tableau et ajouter des lignes de séparation
kable(data_description, col.names = c("Caractéristique", "Description", "Type"), format = "html") %>%
...
- **[Source 🚀](https://www.kaggle.com/datasets/antaesterlin/walmart-commerce-data)**

```{r,echo=FALSE, results='asis'}
data_description <- data.frame(
  Caractéristique = c("Row ID", "Order ID", "Order Date", "Ship Date", "Ship Mode", "Customer ID", "Customer Name", "Segment", "Country", "City", "State", "Postal Code", "Region", "Product ID", "Category", "Sub-Category", "Product Name", "Sales", "Quantity", "Discount", "Profit"),
  Description = c(
    "Identifiant de ligne(1, 2...)", 
    "Identifiant de la commande", 
    "Date à laquelle la commande se passe", 
    "Date d’expédition", 
    ...
    "Ville où la commande a été expédiée", 
    "États aux états-unis", 
    "Code postal", 
    "Région géographique(l’est, l’ouest...)", 
    "Identifiant du produit de cette commande", 
    "Catégorie générale(comme Product Line)", 
    "Sous-catégorie spécifique du produit", 
    "Nom du produit", 
    "Montant des ventes de ce produit", 
    "Nombre d'unités de ce produit", 
    "Montant de réduction à ce produit", 
    "Montant de profit généré par ce produit"
  ),
...
# Utilisez la fonction kable pour créer un tableau et ajouter des lignes de séparation verticales et des lignes
kable(data_description, col.names = c("Caractéristique", "Description", "Type"), format = "html") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F) %>%
  column_spec(1, border_right = TRUE) %>%
  column_spec(2, border_right = TRUE) %>%
...
2. **Dimension client** : Quelles sont les unités de clients annuels moyens des supermarchés aux États-Unis et au Myanmar ? Quelle est la relation entre le nombre d’articles achetés par un client en une seule transaction et le prix total ? Comment classer les clients en fonction de leur comportement de consommation et formuler des stratégies correspondantes ?

   Le modèle RFM en gestion de la relation client analyse trois variables : la récence (R), la fréquence (F) et le montant (M). Une valeur R faible indique une probabilité élevée de répétition d'achat et une grande valeur du client. Une fréquence d'achat élevée (F) signale une forte fidélité du client. Le montant total des achats (M) est proportionnel à la fidélité du client. Le score RFM permet ainsi d'estimer la valeur du client.
...
```{r}
# Calculer le montant moyen des transactions pour chaque sous-catégorie et les classer par ordre décroissant du montant moyen des transactions.
...
# Créer des diagrammes à barres pour afficher les résultats du tri
bar_plot <- ggplot(average_transaction, aes(x = reorder(`Sub-Category`, -Average_Transaction_Amount), y = Average_Transaction_Amount)) +
  ...
  labs(
    ...
    y = "Average Transaction Amount (USD)",
    x = "Sub-Category"
  ) +
  theme(axis.text.x = element_text(angle = 65, vjust = 0.6)) # Ajuster l'angle et la position des étiquettes de l'axe des x
...
### 2.1.2 Calcul de la part de marché
...
# : : Tracer des histogrammes de parts de marché
...
- La sous-catégorie des téléphones arrive en tête en termes de parts de marché, avec une part de plus de 15 %, ce qui suggère une forte demande de produits téléphoniques sur le marché américain.
- Les sous-catégories des chaises et des classeurs suivent avec des parts de marché d'un peu moins de 10 % chacune, ce qui reflète probablement la demande constante de mobilier et de fournitures de bureau sur le marché.
- D'autres sous-catégories telles que le stockage, les copieurs et les appareils ménagers ont des parts de marché plus faibles, mais cela ne signifie pas qu'elles n'ont pas de potentiel de croissance. La faible part de marché peut être due à une forte concurrence sur le marché ou à la nature du segment.



```{r echo=FALSE}
# Calculer les ventes totales et la part de marché pour chaque ligne de produits
...
# Visualiser la part de marché sous forme de diagramme circulaire et montrer les pourcentages
...
![Average Transaction Amount by Sub-Category](shiny/screen_capture/R_czy.png)
...
### 2.2.1 Quelles sont les unités de clients annuels moyens des supermarchés aux États-Unis et au Myanmar ?
...
**"Client unit"** se réfère au nombre moyen d'articles achetés par un client en une seule transaction.
...
### 2.2.2 Quelle est la relation entre le nombre d’articles achetés par un client en une seule transaction et le prix total ?
...
**USA**
  ...
  - Le graphique de gauche (USA) montre une augmentation générale du prix total avec l'augmentation de la quantité d'articles achetés.
  - Cependant, la dispersion est plus importante pour les petites quantités (2 à 7 articles), avec des valeurs totales qui varient largement.
  - Pour des quantités plus grandes (à partir de 10 articles), le prix total semble se stabiliser autour de 500 à 750 $, mais avec une plus grande variabilité pour les très grandes quantités (16-19 articles).

**MYA**

  - Le graphique de droite (MYA) montre une augmentation plus linéaire et plus claire du prix total avec l'augmentation de la quantité d'articles achetés.
  - La médiane du prix total augmente régulièrement avec la quantité, ce qui indique une relation plus directe et proportionnelle entre la quantité et le prix total par rapport aux USA.
  - La variabilité du prix total augmente également avec la quantité, mais de manière plus uniforme par rapport au graphique des USA.

**Conclusion**

  - **Relation Positive** : Dans les deux pays, il y a une relation positive entre la quantité d'articles achetés et le prix total. Plus un client achète d'articles en une seule transaction, plus le prix total a tendance à être élevé.
  - **Variabilité** : La variabilité du prix total est plus marquée dans le cas des USA, surtout pour des petites et grandes quantités, alors que pour MYA, l'augmentation est plus linéaire et la variabilité augmente de manière plus prévisible.
  - **Différences Régionales** : Les clients aux USA montrent une plus grande dispersion des prix pour une même quantité d'articles par rapport aux clients en MYA, ce qui peut être dû à des facteurs tels que les différences de prix des articles, les promotions ou les comportements d'achat.

### 2.2.3 Comment classer les clients en fonction de leur comportement de consommation et formuler des stratégies correspondantes ?
...
![RFM model](shiny/screen_capture/rfm.jpg)
...
- **Récence (R)** : date à laquelle un client a effectué un achat (jours depuis le dernier achat). Calculez le nombre de jours depuis le dernier achat du client. Attribuez un score en fonction de la date d'achat, les achats plus récents obtenant des scores plus élevés.
- **Fréquence (F)** : fréquence à laquelle un client effectue un achat (jours à quelle fréquence faire ses achats). Comptez le nombre total d'achats effectués par le client au cours d'une période de temps spécifiée. Attribuez un score en fonction du nombre d'achats, avec des acheteurs plus fréquents. obtenir des scores plus élevés.
- **Monétaire (M)** : combien d'argent un client dépense (montant de l'achat en dollars américains). Additionnez le montant total d'argent que le client a dépensé au cours d'une période de temps spécifiée. Attribuez un score en fonction des dépenses totales, les dépensiers les plus élevés obtenant des scores plus élevés.
...
1. **R2F1M1 - New Customers**
  - **Description** : Il s'agit de clients qui viennent d'effectuer leur premier achat récemment. Leur fréquence d'achat et leur montant sont tous deux faibles.
  - **Caractéristiques** : Puisqu’il s’agit de nouveaux clients, ils doivent devenir des clients fidèles grâce à un bon service et des stratégies marketing appropriées.
2. **R2F1M2 - Important Deepening Customers**
  - **Description** : Ces clients ont récemment effectué leur premier achat et, bien que la fréquence d'achat soit faible, le montant de l'achat est élevé.
  - **Caractéristiques** : ces clients ont un potentiel élevé et ont besoin de mieux comprendre leurs besoins et de fournir des produits et services personnalisés pour augmenter leur fréquence d'achat.
3. **R2F2M1 - Potential Customers**
  - **Description** : Ces clients ont récemment effectué des achats, avec une fréquence d'achat élevée, mais un faible montant par achat.
  - **Caractéristiques** : ces clients ont une fréquence d'interaction élevée et le montant de leurs achats peut être augmenté en recommandant des produits connexes et en améliorant l'expérience client.
4. **R2F2M2 - Important Value Customers**
  - **Description** : Ces clients ont récemment effectué des achats fréquents et disposent d'un montant élevé.
  - **Caractéristiques** : Ce sont les clients les plus précieux et doivent être entretenus et pris en charge, avec des offres exclusives et des services de qualité pour maintenir leur fidélité.
5. **R1F1M1 - Churned Customers**
  - **Description** : Ces clients n'ont pas effectué d'achats depuis longtemps et ont une fréquence et un montant d'achat faibles.
  - **Caractéristiques** : ces clients risquent de perdre leur clientèle et devront peut-être être réactivés grâce à des promotions spéciales ou à des stratégies de réactivation.
6. **R1F1M2 - Important Retention Customers**
  - **Description** : Ces clients n'ont pas effectué d'achats depuis longtemps, mais ils avaient auparavant des montants d'achat élevés.
  - **Caractéristiques** : Ces clients étaient autrefois des clients de grande valeur et ont besoin d'être restimulés grâce à des soins particuliers et à des stratégies promotionnelles personnalisées.
7. **R1F2M1 - Average Maintenance Customers**
  - **Description** : Ces clients ont un comportement d'achat moyen, avec une fréquence élevée mais un faible montant.
  - **Caractéristiques** : Ces clients constituent un groupe de clients relativement stable et leur comportement d'achat peut être maintenu grâce à une interaction continue et des offres modérées.
8. **R1F2M2 - Important Win-Back Customers**
  - **Description** : Ces clients ont une fréquence d’achat élevée et un montant d’achat élevé, mais n’ont effectué aucun achat récemment.
  - **Fonctionnalités** : Ces clients étaient autrefois des clients très importants, et vous pouvez essayer de les réactiver en développant des stratégies de récupération ciblées (telles que des offres spéciales et des recommandations personnalisées).
...
#### 2.3.1.2 L'édition de shiny

Merci de dragger le barre pour changer la gamme de profit.
Et Veuillez cocher ou décocher les checkboxs pour limiter la catégorie des produits.

![unitPrice_sales](shiny/screen_capture/4.png)
...
#### 2.3.2.3 L'édition de shiny

Merci de dragger le barre pour changer la gamme de vente.
Et Veuillez cocher ou décocher les checkboxs pour limiter la catégorie des produits.

![unitPrice_profit](shiny/screen_capture/5.png)
---
...
```{r}
# Nettoyage des données: convertir le format de date
data_usa$`Order Date` <- dmy(data_usa$`Order Date`)
data_usa$`Ship Date` <- dmy(data_usa$`Ship Date`)

# Convertir la date au format "année-mois-jour"
...
# Créer un champ YearMonth pour le regroupement
data_usa$YearMonth <- format(as.Date(data_usa$`Order Date`), "%Y-%m")
# Vérifiez s'il n'y a pas de données
...
# Résumer les données dans la table Monthly_sales
...
```{r}
# Utiliser ggplot pour dessiner un graphique linéaire - Profit et Quantity
...
# Utilisez ggplot pour dessiner un graphique linéaire - SumPrice
...
# Agréger les données au niveau du trimestre
quarterly_sales <- data_usa %>%
  group_by(Quarter) %>%
  summarise(TotalSales = sum(Sales))

# Créez un objet de série chronologique, les données commencent à partir de 2014, la fréquence est de 4 (données trimestrielles)
sales_ts <- ts(quarterly_sales$TotalSales, start = c(2014, 1), frequency = 4)

# Utilisation de l'ajustement automatique du modèle ARIMA
fit <- auto.arima(sales_ts)

# Informations récapitulatives sur la vue du modèle
summary(fit)

# Prévisions de ventes 2018 (4 trimestres)
...
Prévisions ponctuelles: représente les ventes moyennes prévues pour chaque trimestre.
Lo 80 et Hi 80:indiquent l'intervalle de confiance de 80%, ce qui signifie que la valeur prédite a une probabilité de 80% de se situer dans cette plage.
Lo 95 et Hi 95:indiquent l'intervalle de confiance de 95%, ce qui signifie que la valeur prédite a une probabilité de 95% de se situer dans cette plage.


#### 2.4.2.3 Les images suivantes sont des résultats de prédiction :

```{r}
# Visualisez les résultats des prédictions
autoplot(forecast) + 
...
```{r}
# Filtrer les clients des États-Unis
...
# Sélectionner les 10 premières villes par nombre de clients
top_city_customers <- city_customers %>% top_n(10, CustomerCount)

# Tracer le nombre de clients dans les 10 premières villes
ggplot(top_city_customers, aes(x = reorder(City, CustomerCount), y = CustomerCount)) +
  geom_bar(stat = "identity", fill = "skyblue") +
  coord_flip() +
  labs(
    title = "Top 10 des villes par nombre de clients aux États-Unis",
    x = "Ville",
...
# Sélectionner les 10 villes avec les ventes moyennes les plus élevées
...
ggplot(top_city_avg_sales, aes(x = reorder(City, AverageSales), y = AverageSales)) +
  ...
  coord_flip() +
  labs(title = "Top 10 des villes par ventes moyennes aux États-Unis", x = "Ville", y = "Ventes moyennes (USD)") +
  scale_y_continuous(labels = scales::dollar_format(prefix = "$")) +
  theme_minimal()

```

![1](./shiny/screen_capture/1.jpg)
...
Jamestown est la ville avec les ventes moyennes les plus élevées, bien au-dessus des autres villes.
Cheyenne et Bellingham suivent respectivement en deuxième et troisième positions.
D'autres villes comme Independence, Burbank, etc., sont également parmi les 10 premières.
Ce graphique aide à comprendre quelles villes ont les clients qui dépensent le plus en moyenne par transaction, ce qui est précieux pour l'élaboration de stratégies de marketing et de vente.
...
Zhaoyang Chen était chargé de l'analyse des montants moyens des transactions et de la détermination des parts de marché. Les travaux spécifiques comprennent, sans toutefois s'y limiter :

- Calcul des montants moyens des transactions pour chaque sous-catégorie et classement de ces montants par ordre décroissant. Création de diagrammes à barres pour visualiser ces informations.
- Analyse des parts de marché pour diverses sous-catégories de produits en utilisant les données du marché américain, et visualisation des résultats à travers des histogrammes. Des analyses similaires ont été réalisées pour le marché de Myanmar, avec des visualisations sous forme de diagrammes circulaires pour afficher les parts de marché par ligne de produits.

### 4.1.3 Zhifan Lin

J'ai effectué l'analyse des problèmes liés aux clients en utilisant le modèle RFM et en le présentant via le framework Shiny. Les travaux spécifiques incluent, sans toutefois s’y limiter :

- Nettoyage des données, y compris la suppression des valeurs manquantes (NA) et des données clients présentant des écarts importants.
- Analyse des comportements d'achat des clients et de leur valeur.
- Développement de graphiques interactifs avec Shiny pour visualiser les scores RFM des clients.

### 4.1.4 Wuyuan XU

J'ai effectué l'analyse des problèmes liés aux produits en utilisant le ggplot2 et le shiny. Mes 2 graphiques ggplot révèlent respectivement la relation entre la vente et le prix unitaire et celle entre le profit et le prix unitaire. Puis je les ai transformés aux graphiques interactifs avec l'application shiny. Et la partie tableau dans le slide de présentation était aussi faite par moi. 

- Nettoyage des données, en incluant le regroupement par produit ID et catégorie via la fonction d'aggrégation, et la calculation du champ prix unitaire.
- Analyse liée à la dimension produit dans les deux pays différents.
- Création des graphiques interactifs avec le shiny pour ajouter d'autres dimensions. Par exemple, tu peux dragger le barre ou cocher les checkbox pour voir l'influence d'autres dimensions sur notre analyse originale. 
