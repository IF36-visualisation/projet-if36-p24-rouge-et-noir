...
2. **Dimension client** : 
  - Quelles sont les unités de clients annuels moyens des supermarchés aux États-Unis et au Myanmar ?
  - Quelle est la relation entre le nombre d’articles achetés par un client en une seule transaction et le prix total ?
  - Comment classer les clients en fonction de leur comportement de consommation et formuler des stratégies correspondantes ?
...
```{r}
# Tout d'abord, nous devons calculer le total des ventes et des bénéfices pour chaque sous-catégorie.
category_summary <- data_usa %>%
  group_by(`Sub-Category`) %>%
  summarise(Total_Sales = sum(Sales, na.rm = TRUE),
            Total_Profit = sum(Profit, na.rm = TRUE)) %>%
  arrange(desc(Total_Sales)) # Par ordre décroissant des ventes totales

# Créer un diagramme à barres bi-axial
p <- ggplot(category_summary, aes(x = reorder(`Sub-Category`, Total_Sales))) +
  ...
  labs(title = "Total Sales and Profit by Sub-Category",
       ...
       fill = "Bar Type") + # Ajouter une légende pour le type de barre
  theme_minimal() +
  theme(
...
**Conclusion**
  ...
  - **Variabilité** : La variabilité du prix total est plus marquée dans le cas des USA, surtout pour des petites et grandes quantités, alors que pour la Malaisie, l'augmentation est plus linéaire et la variabilité augmente de manière plus prévisible.
  - **Différences Régionales** : Les clients aux USA montrent une plus grande dispersion des prix pour une même quantité d'articles par rapport aux clients en Malaisie, ce qui peut être dû à des facteurs tels que les différences de prix des articles, les promotions ou les comportements d'achat.
...
### 2.3.1 La relation entre les ventes et le prix répond aux 2 questions : Quelle gamme de prix de produits se vend le mieux ? Quel niveau de prix génère réellement le plus de ventes ?
...
### 2.3.2 La distribution des prix et des bénéfices est utile pour comprendre la sensibilité au prix et la compétitivité des différents marchés. Il est évident que dans les deux pays, un prix plus élevé entraîne un bénéfice plus important, même s'il est plus difficile de trouver la tendance des points bleus (les produits aux États-Unis).

#### 2.3.2.1 Sensibilité au prix : La sensibilité au prix du marché peut être comprise en examinant la distribution et les tendances des prix des produits. 
...
#### 2.3.2.2 La compétitivité : La compétitivité du marché peut être appréhendée en examinant la distribution et les tendances des bénéfices sur les produits. 
