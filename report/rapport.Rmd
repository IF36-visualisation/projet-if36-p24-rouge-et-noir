...
```{r include = FALSE}
data_usa <- read_csv("superstore.csv")
head(data_usa)
```

```{r include = FALSE}
data_mya <- read_delim("WalmartSQL repository.csv", 
...
client_unit_year_usa <- client_usa %>%
  ...
  summarise(client_unit = sum(total) / n())
...
max_y <- (max(max(client_unit_year_usa$client_unit), max(client_unit_year_mya$client_unit)) %/% 100 + 1.5) * 100
...
bar_client_unit_usa <- ggplot(client_unit_year_usa, aes(x = year, y = client_unit)) +
  ...
  labs(title = "Average Client Unit by Year (USA)", x = "Year", y = "Average Client Unit") +
...
bar_client_unit_mya <- ggplot(client_unit_year_mya, aes(x = year, y = client_unit)) +
  ...
  labs(title = "Average Client Unit by Year (MYA)", x = "Year", y = "Average Client Unit") +
  coord_cartesian(ylim = c(0, max_y)) +
  scale_y_continuous(expand = c(0, 0)) +
  theme(plot.title = element_text(size = 12)) + 
  geom_text(aes(label = round(client_unit, digits = 1)), 
